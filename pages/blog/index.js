        ...
        blogs.map((blog, i)=>{
          return(
            <div key={i} className="_blog col-md-6 col-lg-4">
              <Image src={blog.node.featuredImage?.node.mediaItemUrl} alt={blog.node.title} width={500} height={500}/>
              <Link href={`/blog/${blog.node.slug}`}><h4 className="">{blog.node.title}</h4></Link>
            </div>
          )
        })
      }
...
export async function getServerSideProps() {
  const GET_POSTS = gql`${getAllBlogs}`;
